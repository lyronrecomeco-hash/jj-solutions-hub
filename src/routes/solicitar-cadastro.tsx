...
import { ArrowLeft, CheckCircle2, Loader2, Upload, X, ChevronRight, FileText } from "lucide-react";
...
function SignupPage() {
  ...
  const [form, setForm] = useState({
    first_name: "", last_name: "", cpf: "", rg: "", email: "", phone: "", birth_date: "",
  ...
  function onDoc(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f.size > 8 * 1024 * 1024) { toast.error("Arquivo grande demais (máx. 8MB)"); return; }
    if (!/^(image\/|application\/pdf)/.test(f.type)) { toast.error("Envie JPG, PNG ou PDF"); return; }
    setDoc(f);
    setDocPreview(f.type.startsWith("image/") ? URL.createObjectURL(f) : null);
  }

  async function submit() {
    if (!validateStep(1) || !validateStep(2)) return;
    if (!doc) { toast.error("Envie o documento com foto (RG ou CNH)"); return; }
    setSubmitting(true);
    try {
      const ext = doc.name.split(".").pop() || "jpg";
      const path = `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
      const { error: upErr } = await supabase.storage.from("signup-documents").upload(path, doc, { upsert: false });
      if (upErr) throw new Error(upErr.message);
      const { data: pub } = supabase.storage.from("signup-documents").getPublicUrl(path);
  ...
  function validateStep(target: Step) {
    if (target === 1) {
      if (!form.first_name.trim() || !form.last_name.trim() || !form.email.trim() || !form.phone.trim() || !form.birth_date) {
          ...
          {done ? (
            <div className="rounded-2xl border border-border bg-surface p-10 text-center shadow-soft">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
                Sua solicitação está <strong>em análise</strong>. Quando aprovada, um administrador
                gerará seu acesso e enviará as credenciais por e-mail.
              </p>
                ...
                {step === 3 && (
                  <div className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <Select value={docType} onValueChange={(v) => setDocType(v as "rg" | "cnh")}>
                    ...
                    <div className="rounded-xl border-2 border-dashed border-border bg-surface-muted/30 p-5">
                      <Label className="text-xs">Documento com foto *</Label>
                      <p className="mt-1 text-[11px] text-muted-foreground">Foto legível da frente do documento. JPG, PNG ou PDF até 8MB.</p>
                      <div className="mt-3 flex items-center gap-4">
                        <div className="relative h-24 w-32 overflow-hidden rounded-lg border border-border bg-surface">
                          {docPreview ? (
                            <img src={docPreview} alt="" className="h-full w-full object-cover" />
                          ) : doc ? (
                            <div className="grid h-full w-full place-items-center"><FileText className="h-8 w-8 text-muted-foreground" /></div>
                          ) : (
                            <div className="grid h-full w-full place-items-center text-[10px] text-muted-foreground">Sem arquivo</div>
                          )}
                          {doc && (
                            <button type="button" onClick={() => { setDoc(null); setDocPreview(null); }} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-destructive text-destructive-foreground">
                              <X className="h-3 w-3" />
                            </button>
                          )}
                        </div>
                        <Label className="cursor-pointer">
                          <span className="inline-flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-xs font-medium hover:bg-accent">
                            <Upload className="h-3.5 w-3.5" /> {doc ? "Trocar arquivo" : "Selecionar arquivo"}
                          </span>
                          <input type="file" accept="image/*,application/pdf" className="hidden" onChange={onDoc} />
                        </Label>
                      </div>
                      {doc && <p className="mt-2 truncate text-[11px] text-muted-foreground">{doc.name}</p>}
                    </div>
...
function isAdult(date: string) {
  const birth = new Date(`${date}T00:00:00`); if (Number.isNaN(birth.getTime())) return false;
  const limit = new Date(); limit.setFullYear(limit.getFullYear() - 18); return birth <= limit;
}
function isValidCpf(value: string) {
  const cpf = onlyDigits(value); if (cpf.length !== 11 || /^(\d)\1+$/.test(cpf)) return false;
